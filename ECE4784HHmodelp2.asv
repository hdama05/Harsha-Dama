...
stimuli = ((length(t)-1)/200);% This is the length of the stimuli which is 
                              % scaled to be 0.5ms of the total 100ms of
                              % time ie 100/200 = 0.5
xs = 0; % Used below to determine the full length of the stimulation in the loop
...
for i = 2:length(t)
    ...
    %Stimulated
    if(i>(length(t)-1)*0.1) % starts at 10% of time
        if(xs<stimuli)% stimuli is calculated above 
            Ii(i) = 5;% Injection current equals 5 if conditions are correct
            xs = xs+1;
        else
            Ii(i) = 0;
        end
    ...
    % Derivative equation for voltage scaled be step size "time".
...
% Each of the voltage values in the vector have the resting voltage of
% -70 added back to them since the initial value started a 0 instead of
% -70mV.
